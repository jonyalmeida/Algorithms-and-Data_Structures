...
function buildFriendsList(seed, distance, list) {
    ...
    for (let i = 1; i < distance; i += 1) {
        ...
        for (let name of previous) {
            friendsLists[i] = Array.from(
                new Set(friendsLists[i].concat(list[name]))
            );
        }
    }
...
describe("friendsOf()", () => {
    ...
    context(
        "when called with a name that exists in the dependency list",
        () => {
            ...
            it("returns the persons friends and friends of friends for distance 2", () => {
                ...
                const expected = Array.from(
                    new Set(
                        Array.from(friendsList).concat(Array.from(secondList))
                    )
                );
                expected.sort();
            ...
            it("returns all reachable people for an arbitrarily large distance", function () {
                ...
                const friendsLists = buildFriendsList(
                    list[target],
                    distance,
                    list
                );
